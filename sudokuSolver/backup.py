...
class Sudoku:
    ...
    def bts(self):
        return self.btsHelper(deepcopy(self.domains))
    
    def btsHelper(self, domains):
        if self.isComplete():
            return True

        board = deepcopy(self.board)
        key = self.select_unassigned_variable(domains)
    ...
    def forwardCheck(self, key, value, domains):
        inferences = {}
        for constraint in self.constraints[key]:
            if value in domains[constraint]:
                domains[constraint].remove(value)
                if len(domains[constraint]) == 0:
                    domains[constraint].append(value)
                    return {}
                if len(domains[constraint]) == 1:
                    inferences[constraint] = value
        return inferences
    ...
    def select_unassigned_variable(self, domains):
        ...
        for key in self.board:
            if self.board[key] == 0 and len(domains[key]) < minValue:
                minIndex = key
                minValue = len(domains[key])
        return minIndex

    def isComplete(self):
        for key in self.board:
            if self.board[key] == 0 and not self.isConsistent(self.board[key], key):
                return False
        return True
